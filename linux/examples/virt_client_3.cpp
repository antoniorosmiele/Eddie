...
#define N_Camera 3
#define N_Microphone 3
#define N_Monitor_Video 3
#define N_Audio_Speaker 3
...
std::vector<Link> filterConstraint(std::vector<Link> * links)
{
    ...
    for (auto &link : *links)
    {
        ...
        //apply constraint
        if (rt == "eddie.r.camera")
        {   
            if (attrs->find("place")->second == "CR")
                newLinks.push_back(link);
        }
        else if(rt == "eddie.r.microphone")
        {   
            if (attrs->find("place")->second == "CR")
                newLinks.push_back(link);
        }
        else if (rt == "eddie.r.video")
        {   
...
std::vector<std::string> createConstraint(std::vector<Link> * links)
{
    ...
    k+= "= " + std::to_string(N_Monitor_Video);
    ...
    k+= "= " + std::to_string(N_Audio_Speaker);
    ...
    for (auto &link : *links)
    {
        ...
        if (rt == "eddie.r.camera")
        {
            max+= " x" + std::to_string(i) + " * " + attrs->find("resolution")->second + " +";
        }
    ...
    cmp+= "= " + std::to_string(N_Monitor_Video);

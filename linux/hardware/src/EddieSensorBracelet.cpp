...
EddieSensorBracelet::EddieSensorBracelet(const std::string &path,
    ...
    this->ConnectedTo = root["ConnectedTo"].asString();

    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&owner=" + root["owner"].asString() + "&ConnectedTo=" + root["ConnectedTo"].asString();
...
EddieSensorBracelet::~EddieSensorBracelet() {
    delete[] path;
    delete[] attributes;
}

message_t EddieSensorBracelet::render_get(request_t &request) {
    message_t response;
    ...
    if (query["owner"] == "true") answer["owner"] = this->owner;
    if (query["json"] == "true") answer["json"] = this->json_descriptor;
    if (query["name"] == "true") answer["name"] = this->name;
    if (query["ConnectedTo"] == "true") answer["ConnectedTo"] = this->ConnectedTo;
...
message_t EddieSensorBracelet::render_post(request_t &request) {
    message_t response;
...
message_t EddieSensorBracelet::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieSensorBracelet::get_path() {
    return &split_path[0];
}

const char *const *EddieSensorBracelet::get_attributes() {

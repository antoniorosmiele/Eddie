...
EddieCalculatorCPU::EddieCalculatorCPU(const std::string &path,
    ...
    this->nCore = root["nCore"].asString();

    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&deviceID=" + root["deviceID"].asString() + "&nCore=" + root["nCore"].asString();
...
EddieCalculatorCPU::~EddieCalculatorCPU() {
    delete[] path;
    delete[] attributes;
}

message_t EddieCalculatorCPU::render_get(request_t &request) {
    message_t response;
    ...
    if (query["nCore"] == "true") answer["nCore"] = this->nCore;
...
message_t EddieCalculatorCPU::render_post(request_t &request) {
    message_t response;
...
message_t EddieCalculatorCPU::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieCalculatorCPU::get_path() {
    return &split_path[0];
}

const char *const *EddieCalculatorCPU::get_attributes() {

...
EddieDataset::EddieDataset(const std::string &path,
    ...
    this->typeData = root["typeData"].asString();

    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&deviceID=" + root["deviceID"].asString() + "&typeData=" + root["typeData"].asString();
...
EddieDataset::~EddieDataset() {
    delete[] path;
    delete[] attributes;
}

message_t EddieDataset::render_get(request_t &request) {
    message_t response;
    ...
    if (query["typeData"] == "true") answer["typeData"] = this->typeData;
...
message_t EddieDataset::render_post(request_t &request) {
    message_t response;
...
message_t EddieDataset::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieDataset::get_path() {
    return &split_path[0];
}

const char *const *EddieDataset::get_attributes() {

...
EddieMonitorVideo::EddieMonitorVideo(const std::string &path,
    ...
    this->audioConnected = root["audioConnected"].asString();

    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&place=" + root["place"].asString() + "&audioConnected=" + root["audioConnected"].asString();
...
EddieMonitorVideo::~EddieMonitorVideo() {
    delete[] path;
    delete[] attributes;
}

message_t EddieMonitorVideo::render_get(request_t &request) {
    message_t response;
    ...
    if (query["audioConnected"] == "true") answer["audioConnected"] = this->audioConnected;
...
message_t EddieMonitorVideo::render_post(request_t &request) {
    message_t response;
...
message_t EddieMonitorVideo::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieMonitorVideo::get_path() {
    return &split_path[0];
}

const char *const *EddieMonitorVideo::get_attributes() {

...
EddieAllarmSilent::EddieAllarmSilent(const std::string &path,
    ...
    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&owner=" + root["owner"].asString();
...
EddieAllarmSilent::~EddieAllarmSilent() {
    delete[] path;
    delete[] attributes;
}

message_t EddieAllarmSilent::render_get(request_t &request) {
    message_t response;
...
message_t EddieAllarmSilent::render_post(request_t &request) {
    message_t response;
...
message_t EddieAllarmSilent::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieAllarmSilent::get_path() {
    return &split_path[0];
}

const char *const *EddieAllarmSilent::get_attributes() {

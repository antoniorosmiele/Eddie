...
EddieCamera::EddieCamera(const std::string &path,
    ...
    this->resolution = root["resolution"].asString();

    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&place=" + root["place"].asString() + "&resolution=" + root["resolution"].asString();
...
EddieCamera::~EddieCamera() {
    delete[] path;
    delete[] attributes;
}

message_t EddieCamera::render_get(request_t &request) {
    message_t response;
    ...
    if (query["resolution"] == "true") answer["resolution"] = this->resolution;
...
message_t EddieCamera::render_post(request_t &request) {
    message_t response;
...
message_t EddieCamera::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieCamera::get_path() {
    return &split_path[0];
}

const char *const *EddieCamera::get_attributes() {

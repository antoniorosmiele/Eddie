...
EddieTemperatureSensor::EddieTemperatureSensor(const std::string &path,
    ...
    std::string attributest = "rt=" + root["rt"].asString() + "&id=" + root["id"].asString() + "&place=" + root["place"].asString() + "&accuracy=" + root["accuracy"].asString();
...
EddieTemperatureSensor::~EddieTemperatureSensor() {
    delete[] path;
    delete[] attributes;
}

message_t EddieTemperatureSensor::render_get(request_t &request) {
    message_t response;
    ...
    if (query["accuracy"] == "true") answer["accuracy"] = this->accuracy;
...
message_t EddieTemperatureSensor::render_post(request_t &request) {
    message_t response;
...
message_t EddieTemperatureSensor::render_put(request_t &request) {
    return render_post(request);
}

const char *const *EddieTemperatureSensor::get_path() {
    return &split_path[0];
}

const char *const *EddieTemperatureSensor::get_attributes() {
